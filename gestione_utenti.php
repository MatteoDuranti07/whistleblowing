...
try {
    ...
    $utenti = $pdo->query("SELECT id, nome, username, ruolo, data_registrazione FROM utenti")->fetchAll(PDO::FETCH_ASSOC);
...
<head>
  ...
  <style>
    ...
    .badge-admin  { background: #e3ecf5; color: #0d47a1; }
    .badge-utente { background: #e8f5e9; color: #2e7d32; }
    .action-link {
      color: #1976d2;
      margin-right: 12px;
      font-size: 1rem;
    ...
    .action-link:hover { color: #0d47a1; }
    .action-link.delete { color: #e53935; }
    .action-link.delete:hover { color: #b71c1c; }
    .add-btn {
      display: inline-flex;
      align-items: center;
      gap: 8px;
    ...
    <table>
      ...
      <tbody>
        ...
        <tr id="riga-utente-<?= $utente['id'] ?>">
          ...
          <td>
            <a href="modifica_utenti.php?id=<?= $utente['id'] ?>" class="action-link" title="Modifica">
  ...
  function eliminaUtente(el) {
    ...
    .then(data => {
      if (data.success) {
        ...
        setTimeout(() => riga.remove(), 300);
      } else {
        alert('Errore: ' + (data.message || 'Impossibile eliminare l\'utente.'));
      }
    })
